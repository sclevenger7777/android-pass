...
class ObserveEncryptedSharedItemsPagingImpl @Inject constructor(
    ...
    ): Flow<PagingData<ItemEncrypted>> = observeCurrentUser().flatMapLatest { user ->
        when (itemSharedType) {
            ItemSharedType.SharedByMe -> itemRepository.observeSharedByMeEncryptedItemsPaging(
                userId = user.userId,
                itemState = itemState,
                includeHiddenVault = includeHiddenVault
            )

            ItemSharedType.SharedWithMe -> itemRepository.observeSharedWithMeEncryptedItemsPaging(
                userId = user.userId,
                itemState = itemState,
                includeHiddenVault = includeHiddenVault
            )
