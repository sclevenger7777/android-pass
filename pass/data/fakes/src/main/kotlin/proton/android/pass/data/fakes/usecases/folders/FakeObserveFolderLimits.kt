...
@Singleton
class FakeObserveFolderLimits @Inject constructor() : ObserveFolderLimits {

    private val flow = MutableStateFlow(
        FolderLimitsData(
            maxCount = FolderLimits.MAX_FOLDERS_PER_VAULT,
            maxChildren = FolderLimits.MAX_FOLDER_WIDTH,
            maxDepth = FolderLimits.MAX_FOLDER_DEPTH
        )
    )

    fun sendValue(value: FolderLimitsData) {
        flow.value = value
    }

    override fun invoke(): Flow<FolderLimitsData> = flow
}
